  ...
  const handleSaveProfile = async () => {
    try {
      await setDoc(doc(db, 'users', uid), {
        ...
        emergencyContacts: userData.contacts,
        emergencyCode: userData.emergencyCode,
      });
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <View className="m-2 flex justify-center items-center">
          ...
          <View className="w-5/6">
            ...
            {currentStep === 2 && (
              <View>
                <Text className="text-xl text-white font-bold mb-5">Emergency Contacts</Text>

                {userData.contacts.map((contact, index) => (
                  <View key={index} className="mb-6 border border-white rounded-xl p-4">
                    <View className="flex flex-row justify-between items-center mb-2">
                      <Text className="text-white font-semibold">Contact {index + 1}</Text>
                      {index > 0 && (
                        <TouchableOpacity
                          onPress={() => {
                    ...
                    >
                      <Text className="text-white text-center">Pick from Contacts</Text>
                    </TouchableOpacity>
                  </View>
                ))}
                ...
                {userData.contacts.length < 5 && (
                  <TouchableOpacity
                    onPress={() => {
                      setUserData({
                        ...userData,
                        contacts: [...userData.contacts, { name: '', phone: '' }],
                      });
                    }}
                ...
                <TouchableOpacity
                  onPress={() => {
                    ...
                    if (firstContact.name.trim() === '' || firstContact.phone.trim() === '') {
                      alert('At least one complete emergency contact is required.');
                    } else {
                      setCurrentStep(3);
                    }
                  }}
            ...
            {currentStep === 3 && (
              <View>
                <Text className="text-xl text-white font-bold mb-5">Secret Emergency Code</Text>
        ...
        >
          <SafeAreaView className="flex-1 bg-white">
            <Text className="text-xl font-bold text-center my-4">Select a Contact</Text>
              ...
              renderItem={({ item }) => (
                <Pressable
                  ...
                  onPress={() => {
                    ...
                    const updated = [...userData.contacts];
                    updated[targetIndex] = { name, phone };
                    setUserData({ ...userData, contacts: updated });
